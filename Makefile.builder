ifndef LOADING_PLUGINS
    ...
    source-debian-mgmt-salt-dev-in: NAME = $(shell $(DEBIAN_PARSER) changelog --package-name $(ORIG_SRC)/$(DEBIAN_BUILD_DIRS)/changelog)
    source-debian-mgmt-salt-dev-in: ORIG_FILE = "$(CHROOT_DIR)/$(DIST_SRC)/../$(NAME)_$(VERSION).orig.tar.gz"
    source-debian-mgmt-salt-dev-in:
